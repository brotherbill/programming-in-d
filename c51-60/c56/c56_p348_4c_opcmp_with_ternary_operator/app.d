...
void main()
{
	...
	assert(variable0 >= variable1); // Clock >= AlarmClock lexically, because "Clock" >= "AlarmClock"
...
class Clock
{
	...
	override int opCmp(Object o) const
	{
		...
		{
			return typeid(this).opCmp(typeid(o)); // comparing class names lexically as strings.  "AlarmClock" < "Clock"
		}
	...
	// For those that enjoy nesting ternary operators, feast on this. 
	enum USE_TERNARY_OPERATOR = true;
	static if (USE_TERNARY_OPERATOR)
	{
	...
	{
		// Avoiding ternary operator nesting.  As for me, the following code is clearer, and uses the same three lines of code.
...
class AlarmClock : Clock
{
	...
	override ulong toHash() const nothrow @trusted
	{
		return ((60 ^^ 4) * hour) + ((60 ^^ 3 * minute) + ((60 ^^ 2) * second)) + (
			60 * alarmHour) + alarmMinute;
	}
