...
// If comparing Clock to AlarmClock or AlarmClock to Clock, then only compare Clock fields of each

void main()
{
	auto variable0 = new Clock(3, 4, 5);
	auto variable1 = new AlarmClock(6, 7, 8, 5, 0);
	assert(variable0 <= variable1); // Compare Clock parts of Clock and AlarmClock
	assert(variable1 >= variable0); // Compare Clock parts of Clock and AlarmClock
...
class Clock
{
	...
	override int opCmp(Object o) const
	{
		...
		writeln(__LINE__, " ", typeid(this));
		writeln(__LINE__, " ", typeid(o));
		writeln;
		...
		{
			// downcast from o to Clock.  If AlarmClock, downcasts to Clock, otherwise null
			rhsClock = cast(Clock)o;

			if (rhsClock is null)
			{
				// The best can do is lexicographic comparison of Type Names
		...
		Clock rhs = rhsClock ? rhsClock : cast(Clock)o;
