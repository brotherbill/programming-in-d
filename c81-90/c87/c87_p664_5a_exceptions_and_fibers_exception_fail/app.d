import std.stdio     : write, writefln, writeln, readln;
...
void main()
{
    ...
    while (!done)
    {
        ...
        case "hi":
            ...
            enforce(flows.length > 0, "Expected at least one flow.");
...
class SignOnFlow : Fiber
{
    ...
    this()
    {
        // Set our 'run' member function as the starting point of the fiber.
        super(&run);
    }
    ...
    void run()
    {
        ...
        // At this point we have collected all information to construct the user.
        // We now "return" instead of 'Fiber.yield()'.
        // As a result, the state of this fiber becomes Fiber.State.TERM.
    }
...
// Identifies the owner fiber for the input, sets its input  data, and resumes that fiber.
// Returns a user with valid fields if the flow has been completed.
User handleFlowData(string line, SignOnFlow[] flows)
{
    ...
    if (flow.state == Fiber.State.TERM)
    {
        ...
        user = flow.user;
        // TODO: This fiber's entry in the 'flows' array can be reused for a new flow in the future.
        // However, it must first be reset by 'flow.reset()'.
    }
    return user;
}
